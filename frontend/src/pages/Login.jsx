...
const Login = () => {
  ...
  useEffect(() => {
    ...
    if (loc.state?.registered) toast.success("Account created. Please sign in.");
  ...
  const sendForgot = () => {
    ...
    toast.success("Reset link sent (mock). Check your email.");
    setForgotOpen(false);
    setForgotEmail("");
  };
    ...
    <div className="min-h-screen flex" style={{ fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-[#0f172a] text-white p-16 flex-col justify-between">
        ...
        <div className="space-y-6">
          <h1 className="text-5xl xl:text-6xl tracking-tighter leading-[0.95]" style={{ fontFamily: "'Cabinet Grotesk', sans-serif" }}>
            Build telecom apps for <span className="text-[#e11d48]">76 million</span> Robi subscribers.
          </h1>
          <p className="text-slate-300 max-w-md leading-relaxed">
            BDapps is the official developer platform for Robi. Provision SMS, USSD, OTP and Subscription services in minutes.
          </p>
        </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-6 sm:px-12 lg:px-20 bg-white">
        <div className="w-full max-w-sm mx-auto">
          ...
          <h2 className="text-3xl font-bold tracking-tight text-[#0f172a] mb-1" style={{ fontFamily: "'Cabinet Grotesk', sans-serif" }}>Sign in</h2>
          <p className="text-sm text-slate-500 mb-8">Welcome back. Enter your credentials.</p>
          ...
          <form onSubmit={submit} className="space-y-4" data-testid="login-form">
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <button type="button" onClick={() => setForgotOpen(true)} data-testid="forgot-password-link"
                  className="text-xs text-[#e11d48] hover:underline">Forgot password?</button>
              </div>
            ...
            <div className="text-center text-sm text-slate-500">
              No account? <Link to="/register" data-testid="goto-register" className="text-[#e11d48] font-medium hover:underline">Create one</Link>
            </div>
          </form>

          <div className="mt-10 p-4 bg-slate-50 border border-slate-200 rounded-md">
            <p className="text-xs uppercase font-bold tracking-widest text-slate-500 mb-2">Demo credentials</p>
      ...
      <Dialog open={forgotOpen} onOpenChange={setForgotOpen}>
        <DialogContent data-testid="forgot-dialog">
          ...
          <DialogFooter>
            ...
            <Button data-testid="forgot-submit" onClick={sendForgot} className="bg-[#e11d48] hover:bg-[#be123c]">Send link</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
